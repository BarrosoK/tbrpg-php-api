...
function JoinRaid($raidId)
{
    ...
    $player = GetPlayer();
    $playerId = $player->id;
    $player-BattleDb = new PlayerBattle();
    ...
    $raid = $gameData['raids'][$raidId]
    if (!$raid) {
        $output['error'] = 'ERROR_INVALID_RAID_DATA';
    } else if (!DecreasePlayerStamina($playerId, 'RAID', $raid['requireStamina'])) {
        $output['error'] = 'ERROR_NOT_ENOUGH_RAID_STAMINA';
    } else {
        $session = md5($playerId . '_' . $raidId . '_' . time());
        $newData = new PlayerBattle();
        $newData->playerId = $playerId;
        $newData->dataId = $raidId;
        $newData->session = $session;
        $newData->save();

        // Stamina
        $staminaTable = $gameData['staminas']['RAID'];
...
function FinishRaid($session, $battleResult, $deadCharacters)
{
    $gameData = \Base::instance()->get('GameData');
    $output = array('error' => '');
    $player = GetPlayer();
